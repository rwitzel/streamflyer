...
public class ProcessEndOfStreamTest {
	...
	@Test
	public void test2013_14_03_ProcessEndOfStream_beAwareOfFlush()
			throws Exception {
	...
	private boolean rewriteContent(boolean flush, String domainPrefix) throws Exception {
		...
		String oldHtml = "<html><body>" + contentPart + oldUrl + contentPart
				+ "</body></html>";
		String expectedNewHtml = "<html><body>" + contentPart + expectedNewUrl
				+ contentPart + "</body></html>";
		String encoding = "UTF-8";

		ByteArrayOutputStream os = new ByteArrayOutputStream();
		long written = rewriteContent(new ReaderInputStream(new StringReader(
				oldHtml)), os, encoding, flush);
		System.out.println("written: " + written);
		System.out.println("oldHtml.length(): " + oldHtml.length());
		System.out.println("expectedNewHtml.length(): "
				+ expectedNewHtml.length());
	...
	protected long rewriteContent(InputStream input, OutputStream output,
			String encoding, boolean flush) throws IOException {
		...
		FastRegexModifier modifier = new FastRegexModifier(regex,
				Pattern.CASE_INSENSITIVE | Pattern.CANON_EQ, replacement);

		Reader reader = new ModifyingReader(new InputStreamReader(input,
				charset), modifier);
