...
public final class Matcher implements MatchResult {
     ...
     * the last match. If hitEnd is true, and a match was found, then more input
     * might cause a different match to be found. If hitEnd is true and a match
     * was not found, then more input could cause a match to be found. If hitEnd
     * is false and a match was found, then more input will not change the
     * match. If hitEnd is false and a match was not found, then more input will
     * not cause a match to be found.
     */
    boolean hitEnd;

    /**
     * Boolean indicating whether or not more input could change a positive
     * match into a negative one. If requireEnd is true, and a match was found,
     * then more input could cause the match to be lost. If requireEnd is false
     ...
     * <code>String</code>. This method produces a <code>String</code> that will
     * work as a literal replacement <code>s</code> in the
     * <code>appendReplacement</code> method of the {@link Matcher} class. The
     * <code>String</code> produced will match the sequence of characters in
     * <code>s</code> treated as a literal sequence. Slashes ('\') and dollar
     ...
     *         capturing group that does not exist in the pattern
     */
    ...
    public StringBuffer appendTail(StringBuffer sb) {
        sb.append(getSubSequence(lastAppendPosition, getTextLength())
                .toString());
        return sb;
    }
     ...
     *         start is greater than the length of the input sequence, if end is
     *         greater than the length of the input sequence, or if start is
     *         greater than end.
     * @return this matcher
     * @since 1.5
     */
     ...
     * it proceeds in this matcher. Matcher.from is not set here, because it is
     * the "hard" boundary of the start of the search which anchors will set to.
     * The from param is the "soft" boundary of the start of the search, meaning
     * that the regex tries to match at that index but ^ won't match there.
     * Subsequent calls to the search methods start at a new "soft" boundary
     * which is the end of the previous match.
     */
