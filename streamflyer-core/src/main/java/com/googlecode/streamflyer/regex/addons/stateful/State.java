...
/**
 * A state of the match process. The state is reached if the corresponding
 * {@link #getToken() token} is matched.
 * <p>
 * The state transitions can be set via
 * {@link #setTransitions(List, TransitionGuard)}.
 * 
 * @author rwoo
 */
public class State {
    ...
    /**
     * If this constructed state is reached, the stream is
     * {@link DoNothingProcessor not modified}.
     * 
     * @param stateName A unique name for the state.
     * @param regex In order to reach this state, this regular expression must
     *        be matched.
     */
    ...
    /**
     * This constructor defines a state so that text of the matched state token
     * is replaced with the given replacement.
     * 
     * @param stateName A unique name for the state.
     * @param regex In order to reach this state, this regular expression must
     *        be matched.
     * @param replacement The replacement defines how the text that is matched
     *        via {@link #regex} shall be replaced.
     */
    ...
    /**
     * If the constructed state is reached then the given {@link MatchProcessor}
     * modifies the stream.
     * 
     * @param stateName A unique name for the state.
     * @param regex In order to reach this state, this regular expression must
     *        be matched.
     * @param matchProcessor If the constructed state is reached then the given
     *        {@link MatchProcessor} modifies the stream.
     */
     ...
     * Convenience method.
     * <p>
     * Sets {@link #transitions} as defined by the reachable states and the
     * transition guard.
     * 
     * @param endStates the states that can be reached from the this state.
     * @param transitionGuard this guard can stop the transition or add
     *        additional logic to the transition.
     */
    public void setTransitions(List<State> endStates,
    ...
    /**
     * @return Returns the token processor that must be used to switch to the
     *         next state.
     */
