...
public class MatchProcessorResult {
    ...
    /**
     * A position in the character buffer. The characters before this position
     * shall be considered unmodifiable by the {@link RegexModifier}.
     * <p>
     * This position must not be smaller than the (originally) first modifiable
     * character in the buffer.
     * <p>
     * This position must not be greater than the size of the buffer plus one.
     * <p>
     * EXAMPLE: This position is equal to the size of the buffer plus one if the
     * empty string is matched (and processed) and the matching shall be
     * continued after the first character after the empty match. This avoids
     * endless loops.
     */
    private int firstModifiableCharacterInBuffer;

    /**
     * True if the matching shall be continued using the characters that already
