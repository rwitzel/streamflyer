...
/**
 * This class creates {@link AfterModification} instances for the use cases that are often needed by implementations of
 * {@link Modifier}.
 * <p>
 * This class creates {@link AfterModification} instances for the most common use cases, see
 * <ul>
...
public class ModificationFactory {
    ...
    public AfterModification skipEntireBuffer(StringBuilder characterBuffer, int firstModifiableCharacterInBuffer,
        ...
        return skipOrStop(characterBuffer.length() - firstModifiableCharacterInBuffer, characterBuffer,
    ...
    /**
     * Skips some characters if there are some in the buffer. This method requires that are modifiable characters
     * somewhere in the stream.
     * 
     * @param numberOfCharactersToSkip
     *            the number of characters to skip. This must greater than zero unless the end of stream is not hit and
    ...
    public AfterModification skip(int numberOfCharactersToSkip, StringBuilder characterBuffer,
        ...
        if (numberOfCharactersToSkip == 0) {
            if (characterBuffer.length() - firstModifiableCharacterInBuffer == 0 && !endOfStreamHit) {
                ...
                String msg = String.format("Probably a programming error. Therefore, "
                        + "we don't fix it automatically." + " (%s, %s, %s, %s)", //
                        characterBuffer, firstModifiableCharacterInBuffer, characterBuffer.length(), endOfStreamHit);
        ...
        if (numberOfCharactersToSkip > numberOfModifiableCharactersInBuffer) {
            String msg = String.format("Probably a programming error. Therefore, "
                    + "we don't fix it automatically. (%s, %s, %s, %s)", characterBuffer,
                    firstModifiableCharacterInBuffer, numberOfCharactersToSkip, characterBuffer.length());
            // System.out.println(msg);
            throw new IllegalStateException(msg);
        }
     ...
     * This method
     * <ul>
     * <li>skips the given characters to use the character buffer as optimal as possible,
     * <li>checks whether skipping and the current value of {@link AfterModification#getNewNumberOfChars()} would fetch
     * more input in the character buffer. If not then {@link AfterModification#getNewNumberOfChars()} is doubled until
     * the number would fetch more input in the character buffer.
     * </ul>
     * 
     * @param numberOfCharactersToSkip
     * @param characterBuffer
    ...
    /**
     * Use this method if you want to request another number of characters from the {@link ModifyingReader} and re-try a
     * modification (MODIFY AGAIN IMMEDIATELY).
     * 
     * @return Returns a modification that requests another number of characters from the {@link ModifyingReader} and
     *         requests a call to {@link Modifier#modify(StringBuilder, int, boolean)} via
     ...
     * @param firstModifiableCharacterInBuffer
     * @param endOfStreamHit
     * @return Returns an {@link AfterModification} that is appropriate if the end of stream is hit and there are no
     *         more modifiable characters and you don't want to add characters at the end of the stream. If there are
     *         still modifiable characters in the stream yet, then this method will throw an exception.
     */
    public AfterModification stop(StringBuilder characterBuffer, int firstModifiableCharacterInBuffer,
        ...
        // validate arguments
        if (characterBuffer.length() - firstModifiableCharacterInBuffer != 0 || !endOfStreamHit) {
            String msg = String.format("Probably a programming error. Therefore, " + "we don't fix it automatically."
                    + " (%s, %s, %s, %s)", //
                    characterBuffer, firstModifiableCharacterInBuffer, characterBuffer.length(), endOfStreamHit);
            // System.out.println(msg);
            throw new IllegalStateException(msg);
